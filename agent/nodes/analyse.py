...
def preprocess_results(
    ...
    if is_adhoc:
        ...
        # Handle all common column name variations from LLM-generated SQL
        def _sum(rows, *keys):
            for k in keys:
        ...
        def _find_col(row, *candidates):
            """Find first matching column regardless of case or naming style."""
        ...
        # Try every reasonable column name variation
        total_orders    = _sum(orders, "totalOrders", "total_orders", "order_count",
                               "num_orders", "orders", "count")
        total_incidents = _sum(orders, "totalIncidents", "total_incidents", "incident_count",
                               "num_incidents", "incidents")
        total_returns   = _sum(orders, "totalReturns", "total_returns", "return_count",
                               "num_returns", "returns")
        total_revenue   = _sum(orders, "totalGrossRevenue", "total_gross_revenue",
                               "gross_revenue", "revenue", "total_revenue")

        # If orders table didn't have incident/return counts, try incidents/returns tables
        if total_incidents == 0 and incidents:
            total_incidents = _sum(incidents, "totalIncidents", "total_incidents",
                                   "incident_count", "count")
        if total_returns == 0 and returns:
            total_returns = _sum(returns, "totalReturns", "total_returns",
                                 "return_count", "count")

        # Detect period comparison columns (e.g. grossRevenue_2026, orders_2025)
        # These are generated by conditional aggregation SQL for period comparisons
        import re as _re
        periods = {}
        if orders:
            first = orders[0]
            ...
            for col in first.keys():
                m = _re.match(r"^(.+?)_(\d{4})$", col)
                if m:
                    metric, year = m.group(1), m.group(2)
                    if year not in period_cols:
                        period_cols[year] = {}
                    period_cols[year][metric] = col

            if period_cols:
                # Build period summaries from conditional aggregation columns
                for year, col_map in sorted(period_cols.items()):
                    p = {"year": year, "total_orders": 0, "total_gross_revenue": 0,
                         "total_net_revenue": 0, "total_incidents": 0, "total_returns": 0,
                         "total_units": 0}
                    for row in orders:
                        for metric, col in col_map.items():
                            val = row.get(col, 0) or 0
                            m_lower = metric.lower()
                            if any(x in m_lower for x in ["order", "cnt", "count"]):
                                p["total_orders"] += val
                            elif any(x in m_lower for x in ["gross", "revenue"]) and "net" not in m_lower:
                                p["total_gross_revenue"] += val
                            elif "net" in m_lower:
                                p["total_net_revenue"] += val
                            elif "incident" in m_lower:
                                p["total_incidents"] += val
                            elif "return" in m_lower:
                                p["total_returns"] += val
                            elif "unit" in m_lower or "qty" in m_lower or "quantity" in m_lower:
                                p["total_units"] += val
                    n = p["total_orders"]
                    p["incident_rate_pct"] = round(p["total_incidents"] / n * 100, 2) if n else 0
                    p["return_rate_pct"]   = round(p["total_returns"]   / n * 100, 2) if n else 0
                    p["total_gross_revenue"] = round(p["total_gross_revenue"], 2)
                    p["total_net_revenue"]   = round(p["total_net_revenue"], 2)
                    periods[year] = p
            else:
                # Fall back to looking for explicit period/timePeriod column
                for row in orders:
                    period = row.get("timePeriod") or row.get("period") or row.get("year") or "All"
                    period = str(period)
        ...
        return {
            ...
            "raw_incidents": incidents[:50],
            "raw_returns":   returns[:50],
        }
...
def analyse_with_llm(
    ...
    system_prompt = (
        ...
        "- Focus ONLY on what was asked in the goal — do not analyse topics not requested\n"
        "- If the goal asks for sales figures, focus on revenue/orders — not incident rates\n"
        "- If the goal asks for a period comparison, interpret by_period data as the comparison\n"
        "- If incidents/returns data is empty but was NOT asked for in the goal, do not flag as failure\n"
        ...
        "- If by_period has entries for multiple years (e.g. 2025 and 2026), treat them as the comparison periods\n"
        "- Do NOT add totals across periods — compare them side by side\n"
        "- Flag clearly if one period has significantly fewer orders than the other (partial year)\n"
        "- The comparison should be like-for-like: same months across years\n"
        "\nCONFIDENCE SCORING GUIDANCE:\n"
        "- Set confidence 0.85-0.95 when: the data requested in the goal is present and non-zero\n"
        "- Set confidence 0.70-0.84 when: data is present but one period may be partial\n"
        "- Set confidence below 0.70 ONLY when: data requested in the goal is missing or zero\n"
        "- Do NOT reduce confidence because incident/return data is absent if it was not requested\n"
        "\nReturn a JSON object with these fields:\n"
        "confidence (float), flags (list), overall_metrics (object), "
        "top_issues (list), by_category (list), by_supplier (list), "
        "improvement_actions (list), anomalies (list)\n"
        "Keep all string values to one sentence maximum."
    )
...
Limit top_issues to 5. Limit by_supplier to top 10 by incident rate. Limit improvement_actions to 5."""
    ...
    goal_instruction = f"User goal: {goal}\n" if goal else ""
    user_prompt = f"""Analyse this supplier performance data and return structured JSON.

Report type: {report_type}
{goal_instruction}{focus}

IMPORTANT: Only analyse what was asked for in the goal. If goal is about sales/revenue comparison,
focus on by_period data. If incidents/returns are empty and were not requested, ignore them.
